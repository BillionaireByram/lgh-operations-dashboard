...
export async function TodayStrip() {
  ...
  const [snap, hybrid] = await Promise.all([
    fromViewOne<TodaySnapshot>("v_today_snapshot"),
    getHybridMetrics({ period: "today", start: today, end: today, label: "Today" }),
  ]);
  const s = snap ?? ({
    ad_spend_today: 0,
  ...
  return (
    <Card title="Today" subtitle={new Date().toUTCString().slice(0, 16)}>
      <div className="grid grid-cols-2 gap-5 md:grid-cols-4 xl:grid-cols-7">
        ...
        <Stat
          label="Hybrid sign-ups"
          value={fmtNum(hybrid.available ? hybrid.registered : s.registrations_today)}
          delta={hybrid.available ? `${hybrid.readyForSetterPush} setter push · ${hybrid.bookingIntent} booking intent` : deltaPct(s.registrations_today, s.registrations_yesterday)}
        />
        ...
        <Stat
          label="Hybrid ready"
          value={fmtNum(hybrid.available ? hybrid.reachedVcc : s.hot_now)}
          sub={hybrid.available ? `${hybrid.needsWatchPush} need watch push` : `${s.open_queue} total leads open`}
          bad={hybrid.available ? hybrid.readyForSetterPush > 0 : s.hot_now > 5}
        />
      </div>
    </Card>
  );
