...
const simpleStep: Record<string, string> = {
  ...
  called: "Called",
  post_registration: "Needs watch",
  post_webinar: "Watched",
  attended: "Watched",
  attended_partial: "Watched",
  attended_75pct: "75%+ watched",
...
function signal(row: CallQueueRow) {
  if (row.watch_minutes && row.watch_minutes >= 56) return `${row.watch_minutes}m watched`;
...
export async function CallQueue() {
  const rows = await fromView<CallQueueRow>("v_call_queue", {
    query: { order: "priority.asc,watch_minutes.desc.nullslast,intent_score.desc.nullslast,registered_at.desc", limit: "100" },
  });
